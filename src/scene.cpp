...
void Cutscene::render(sf::RenderWindow& window) {
    ...
    if (isMenuState) {
        // Draw a fullscreen background for the menu by scaling the texture to window size
        ...
        // Position planet relative to window so it appears correctly in the menu
        ...
        // First draw a fullscreen background (default view) so no black bars appear
        window.setView(window.getDefaultView());
        backgroundSprite.setPosition(0.f, 0.f);
        sf::Vector2u texSize = backgroundTexture.getSize();
        if (texSize.x > 0 && texSize.y > 0) {
            float sx = static_cast<float>(window.getSize().x) / static_cast<float>(texSize.x);
            float sy = static_cast<float>(window.getSize().y) / static_cast<float>(texSize.y);
            // Use max to ensure coverage (no black bars); this may crop horizontally/vertically but avoids empty space
        ...
        // Now draw world objects using the camera view
        window.setView(cameraView);
        // Ensure ship/planet are drawn in world coordinates
        ...
        window.setView(window.getDefaultView());
        // Ensure fade rect covers the window
